...
- **Retries and redirects** — policy-driven backoff with `Retry-After`, replayable-body redirect handling ([retry](docs/concepts/retry.md)). Both are coarse Cargo capabilities (`logical-retry`, `redirects`; Basic auth is `basic-auth`): the lean `native-http1` + `high-level-url` profile omits them for Bearer-only single-attempt clients while `http1`/`http2`/default retain current behavior.
...
The default features are the secure HTTP/1.1 client with Rustls and native roots. See the [feature profile matrix](docs/architecture/feature-flags.md#supported-core-profiles) for minimal, deterministic, and embedded recipes. Low-level embedding without the `url`/`idna`/ICU closure selects `native-http1` (or `native-http2`) without `high-level-url` and uses `Client::execute_http_body` with a caller-owned `http::Uri` (callers own IDNA/punycode conversion). The lean high-level recipe (`native-http1` + `high-level-url` without `logical-retry`/`redirects`/`basic-auth`) keeps the URL/request/response conveniences, Bearer auth, timeouts, body bounds, pooling, TLS, and typed failures while dispatching once and returning 3xx without following.
