...
- Cached forward-proxy and compatible single-target CONNECT clients/connectors contain only connection-scoped policy. Do not pass or retain a logical request's total/deadline state or add it to route keys; the current shrinking request budget is enforced by the outer proxy dispatch timeout and retained behind the private `PoolGuard` response lifecycle through EOF/trailers. The cached CONNECT connector retains only the keyed SNI hint — never wire target overrides, trace observers, read/write budgets, retry/redirect state, bodies, or failure contexts. Preserve this ownership when changing pooling or stale-connection recovery.
- `ResponseBody` public variant shapes (`Buffered { bytes }`, `Streaming { stream, lease }`, `EncodedStreaming { stream, lease, content_encoding, limit }`, `Consumed`) are frozen: never add public timeout fields/variants or `#[non_exhaustive]`. Read/total policy lives behind the private `PoolGuard` response lifecycle and is enforced once at the final selected stream by `BodyTimeoutStream`, which stays the single high-level timeout owner (the focused `ReadTimeoutStream` was removed).
