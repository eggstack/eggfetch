...
- Body sources `--body`/`--body-file`/`--json` are mutually exclusive with `--form`/`--file`; `--form` + `--file` may combine into one multipart body (see the bail message near `main.rs:908`).
