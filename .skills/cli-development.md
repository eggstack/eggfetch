...
- Exit codes are classified in `crates/eggfetch-cli/src/errors.rs` (`map_error_to_exit_code` / `map_unknown_error_to_exit_code`): 0 success, 2 usage (incl. cert/hostname-verification config errors), 3 connect/TLS/pool/proxy transport, 4 timeout, 5 protocol, 6 status (with `--check-status` on any non-2xx), 7 I/O, 130 interrupted via the SIGINT handler in `main.rs`. Do not renumber; keep the mapping typed (no message-substring heuristics).
- Auth/proxy/cookie headers are redacted in verbose output.
- Body sources `--body`/`--body-file`/`--json` are mutually exclusive with `--form`/`--file`; `--form` + `--file` may combine into one multipart body (see the mutual-exclusion bail message in `crates/eggfetch-cli/src/main.rs`).
