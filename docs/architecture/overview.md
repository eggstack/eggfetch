...
| `response` | Yes | `Response`, `HistoryEntry` — incoming response with status, version, headers, URL, body, redirect history, trailers (`trailers()` after EOF). Consumption: `bytes()`, `text()`, `bytes_stream()`, `raw_bytes_stream()`, `text_lines()`. |
...
| `limits` | Yes | `Limits` — logical in-flight limits (`max_in_flight_requests*`, aliases `max_connections*`) + physical idle caps. HTTPX facade keeps `max_connections` naming. |
