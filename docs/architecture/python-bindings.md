...
Async `StreamingResponse.aread()` accumulates chunks in a Rust `BytesMut`
outside the GIL, then freezes and creates the final Python `bytes` inside the
GIL bridge (single copy into Python; the full body is still buffered in Rust,
and a `Bytes` clone is retained for the response cache).
