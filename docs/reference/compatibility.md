...
8. **Timeout semantics**: HTTPX's scalar/default timeout maps to its four operational `connect`, `read`, `write`, and `pool` values. The facade does not create an EggFetch-native `total` deadline; native callers may configure that outer cap explicitly.
