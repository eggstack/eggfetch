...
mod args;
pub(crate) use args::{Cli, Shell};
mod errors;
pub(crate) use errors::{
    map_error_to_exit_code, map_unknown_error_to_exit_code, StatusError, EXIT_IO, EXIT_SUCCESS,
};
mod input;
pub(crate) use input::{
    detect_method, guess_mime, parse_file_part, parse_form, parse_header, parse_query,
    percent_encode, read_body_file,
};
mod output;
...
#[cfg(test)]
use errors::{EXIT_CONNECT, EXIT_PROTOCOL, EXIT_STATUS, EXIT_TIMEOUT, EXIT_USAGE};
#[cfg(test)]
use files::sanitize_filename;
#[cfg(test)]
use output::{format_header_value, is_secret_header};
