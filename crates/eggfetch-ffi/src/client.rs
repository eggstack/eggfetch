...
pub unsafe extern "C" fn eggfetch_client_request(
    ...
    match client.0.request(http_method, url_str) {
        Ok(rb) => Box::into_raw(Box::new(RequestHandle(rb))),
        Err(_) => ptr::null_mut(),
    }
}
...
pub unsafe extern "C" fn eggfetch_client_send(
    ...
    // Safety: request is non-null and valid. We extract the RequestBuilder
    // via ptr::read and forget the box to prevent double-free.
