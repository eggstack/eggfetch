...
use crate::body::{NativeRequestBody, NativeResponseBody, RequestBody, ResponseBody};
    ...
    B::Error: std::error::Error + Send + Sync + 'static,
{
    ...
    #[cfg(feature = "proxy")]
    if resolve_proxy(inner, &logical_url, &crate::request::ProxyOverride::Inherit).is_some() {
        return Err(Error::Unsupported(
    ...
    let uri = resolve_request_uri(&logical_url, &transport_hints)?;
    ...
    let raw_response = match route {
        ...
        TransportRoute::Proxy | TransportRoute::H3 => {
            return Err(Error::Unsupported(
    ...
    let native_body = NativeResponseBody::from_incoming(incoming, Arc::new(guard), timeout.read);
