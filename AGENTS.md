...
The compatibility facade preserves four-phase timeout mappings through native conversion, serializes request parameters exactly once, merges scoped client and request-local cookies without a second native jar, strips explicit Cookie headers on all redirects and regenerates from the client jar, classifies body replay (buffered, reusable-stream, multipart-reconstructable, one-shot) for retained-method redirects, and uses one authoritative body source for each redirect hop. Live sync and async raw iteration marks streams consumed at the pinned first-iteration point, counts raw source bytes before bounded chunk adaptation, handles chunk-size splitting and coalescing, closes on normal exhaustion, and leaves partial finalization/source failure for explicit response close. Native compressed raw parity now uses a core-owned deferred one-shot raw-or-decoded stream boundary; raw selection returns encoded bytes and decoded operations reuse the existing core decompressor. Core's existing decoded-header policy remains bounded: automatic decompression removes `Content-Encoding` and `Content-Length` from response headers, while the compatibility facade overlays only the original wire values for those two headers. Native async cancellation is covered by a deterministic compressed fixture that proves lease release with a constrained follow-up request. Do not claim final closure until `plans/httpx-parity-correction-status.md` records exact implementation, validation, and CI evidence. The compact `test_corrective_kernel.py` is part of Tier 1; the pinned differential module, full pinned-reference compat, and API-oracle validation remain extended.
