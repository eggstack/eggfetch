...
- Native frame execution is additive through `Client::execute_http_body()` and `NativeResponseBody`; it accepts caller-owned `http_body::Body<Data = Bytes>` and shares Hyper/pool/TLS without applying high-level redirects, retries, cookies, auth, decompression, or decoded limits. Existing public body enums remain unchanged. Current hand-rolled proxy and experimental H3 routes reject this surface before body transfer; a 101 upgrade is rejected only after the response status is known, so callers must use the high-level upgrade API when upgrades are possible.
