...
- Native high-level request diagnostics are additive through `Client::send_detailed()` and `RequestBuilder::send_detailed()`. They preserve the existing `Error` enum, `Error::kind()` tokens, and ordinary `send()` behavior while optionally reporting evidence-backed `NetworkFailureKind` values. The request-local classification state is private, bounded, and absent from ordinary requests; it is cleared across retries and safe H3 fallback. Never parse error strings, guess DNS/refusal for opaque routes, or expose transport-library types. See `docs/reference/errors.md` and `docs/architecture/core-engine.md`.
