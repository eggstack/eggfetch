...
- Core request reconstruction goes through typed helpers (`RequestParts::retry_request`, `into_request`, `advance_redirect_hop`) with exhaustive construction — a new field must fail to compile, never be silently dropped. `prepare_single_request()` + `select_route()` (UDS → static/direct → proxy/SOCKS → SNI → H3 → standard) share one post-transport policy for all routes. Static resolved destinations reject proxy, UDS, and H3 combinations before I/O. See `docs/architecture/core-engine.md`.
