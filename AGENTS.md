...
- Performance campaign internals are API-neutral: ordinary H1/H2 dispatch moves prepared method/URI/headers/body into one outgoing request, native frame requests decompose only after validation, SOCKS Hyper responses use `into_parts()`, Python sync streaming uses a bounded async-aware bridge, buffered response headers transfer after metadata extraction, buffered iterators are private lazy objects, and the qualification harness in `scripts/performance_benchmark.py` is non-CI timing evidence. Preserve chunking, cancellation, backpressure, decoding, and public surface contracts when touching these paths.
...
- Live qualification ledger `plans/httpx-parity-correction-status.md` (+ `compat/*/profile.toml`) is authoritative for the current Stage C SHA — never hardcode a SHA outside the canonical qualification records (`docs/residual-differences.md`, `docs/reference/compatibility*.md`). Facade semantics: `docs/architecture/python-bindings.md`.
...
- Python `aread()` must construct the final Python `bytes` object from the collected `Bytes` inside the GIL bridge; do not reintroduce a full-body Rust `Vec` copy. Private buffered iterators must keep independent cursors and preserve scalar-count text chunking, `str.lines()` behavior, and empty-body semantics.
