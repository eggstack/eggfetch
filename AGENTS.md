...
- Native embedded transport controls are additive and Rust-only: `ClientBuilder::dialer()` supplies a raw Tokio stream for the logical host/port while eggfetch owns HTTP and destination TLS; request `target` overrides change only wire path/query; `retry_canceled_requests(false)` controls Hyper's implicit stale-idle retry separately from `RetryPolicy` (default `true`); `PhysicalConnectionPolicy` limits live physical Hyper connections and `TransportIoTimeout` limits established I/O inactivity. The retry flag applies to every Hyper HTTP/1/2 route, not the independent HTTP/3 transport, and does not create Hyper socket-reuse metrics. Custom dialing fails closed with proxy, UDS, resolved-target, local/socket routing, and H3, and never falls back to direct I/O.
